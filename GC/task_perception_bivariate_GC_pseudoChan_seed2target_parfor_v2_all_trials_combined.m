...
homeDir = '/home/plamen/Research/SpeechProduction/EEG';
sourceDir = fullfile(homeDir,'source');
...
subjectID = [4001 4003 4004 4005 4006 4007 4008 4009 4010 4011 4013 4014 4015 4016 4018 4019 4020 4021 4022 4023]
...
tic
GC_derivDir = [homeDir, '/derivatives/BSMART/perception/'];
cd(GC_derivDir)
...
switch PWGC_analyses_type
    case 'full_epoch_window'
        ...
        for subjIdx = 1:length(subjectID)
            ...
            subjName = ['EEGPROD' int2str(subjectID(subjIdx))];
            fprintf('working on %s\n',subjName)
            derivDir = [homeDir, '/derivatives/EEGLAB/perception/' subjName '/' reref_scheme '/' preproc_scheme '/fs_' num2str(fs)];
            cd(derivDir)
...
%                         behavDataFile = sprintf(['EEGPROD',int2str(subjectID(subjIdx)),'_task-perception_0.1_30_sep_1_1_average_ref_-0.2_0.6_popthresh%d.mat'],pop_thresh);
%                     elseif strcmp(reref_scheme, 'mastoid_ref')
%                         behavDataFile = sprintf(['EEGPROD',int2str(subjectID(subjIdx)),'_task-perception_0.1_30_sep_1_1_mastoid_ref_-0.2_0.6_popthresh%d.mat'],pop_thresh);
%                     end
                case 500
                    if strcmp(reref_scheme, 'average_ref')
                        behavDataFile = sprintf(['EEGPROD' int2str(subjectID(subjIdx)) '_task-perception_0.1_30_sep_1_1_average_ref_-0.2_0.6_500Hz_reSample_popthresh%d.mat'],pop_thresh);
                    elseif strcmp(reref_scheme, 'mastoid_ref')
                        behavDataFile = sprintf(['EEGPROD' int2str(subjectID(subjIdx)) '_task-perception_0.1_30_sep_1_1_mastoid_ref_-0.2_0.6_500Hz_reSample_popthresh%d.mat'],pop_thresh);
                    end
            end
                
            preproc_data = load(behavDataFile);
...
%             end
        ...
        GC_results_dir = [GC_derivDir, 'seed_to_target_channels/','pcROI_seed_to_pcROI_target/', ['SW' num2str(sliding_window) '_MO' num2str(model_order) '_fs' num2str(fs) '_3LHSeeds_LHTargets']];
...
fname = sprintf('perception_pwgc_20subj_fs%d_SW%d_M%d_3LHSeeds_LHTargets.mat',fs,sliding_window,model_order);
% fname = sprintf('perception_pwgc_20subj_fs%d_SW%d_M%d_LHSeed[%s]_SriOrthographic_Targets.mat',fs,sliding_window,model_order,seed_combined);
fprintf('saving %s ....... \n\n', fname)
save(fname,'pwgc_mov_seed2target_results','-v7.3');

fname = sprintf('perception_pwgc_20subj_SW%d_M%d_3LHSeeds_LHTargets_key.mat',sliding_window,model_order);
% fname = sprintf('perception_pwgc_20subj_SW%d_M%d_LHSeed[%s]_SriOrthographic_Targets_key.mat',sliding_window,model_order,seed_combined);
fprintf('saving %s ....... \n\n', fname);
